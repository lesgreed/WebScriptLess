...
%***********************************************************
  ...
  for j=1:Npol
    for i=1:Ntor
      ...
      [s,r,B,dBdx,dBdy,dBdz,gradS]=calllib(mconf,'M3DgetdB_Gradsxyz',MC,r,B,dBdx,dBdy,dBdz,gradS);
      ...
      modB1 = sqrt(dot(B,B));
      modB(i,j) = modB1;
      BgrB = [B(1)*dBdx(1) + B(2)*dBdy(1) + B(3)*dBdz(1),B(1)*dBdx(2) + B(2)*dBdy(2) + B(3)*dBdz(2),B(1)*dBdx(3) + B(2)*dBdy(3) + B(3)*dBdz(3)];
                  
      BgrmodB = dot(B,gradB);
      bBgrmodB = B*BgrmodB/modB1;
      bgradb = BgrB - bBgrmodB;     
      bgradb = bgradb/modB1^2;     
                  
      Ncurv(i,j) = dot(bgradb,gradS)/sqrt(dot(gradS,gradS));
    end
  end
  toc;
  figure;
