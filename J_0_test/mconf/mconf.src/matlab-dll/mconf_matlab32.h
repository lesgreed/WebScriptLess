...
#define __CPP2MATLAB_
 ...
 #define __int64 int64_t
  ...
  #endif
#else
typedef unsigned int  MC_HANDLE;
#endif
...
extern "C" {
  ...
  /// @return   on exit the function returns the handle of the object, that holds magnetic configuration,
  /// if the function succeeds, the return value is \b non-zero, \b zero otherwise.
  ...
  /// The function returns the pressure value , where s is the normalized toroidal flux.
  ...
  /// The method returns the Jacobian(s,theta,phi)
  /// Volume element is dV = Jacobian(s,theta,phi)*ds*dtheta*dphi.
  ...
  ///   Boozer Jacobian = FluxTor(s=1)*mu0*(Ip+iota*It)/B^2/twopi^2) \n
  ///   Tokamak Jacobian = FluxTor(s=1)*R*R/(mu0*Ipol) \n
