...
cache = Cache(
    ...
    config={
        ...
        'CACHE_DIR': 'cache-dir',
        'CACHE_DEFAULT_TIMEOUT': 36000  # seconds
    }
)
                        ...
                        children=[
                            html.Div(
                                ...
                                children=[
                                    ...
                                    html.Label("%", className="inline-label")
                                ],
                            ),
                            html.Div(
                                className="input-pair",
                                children=[
                                    ...
                                    dcc.Input(id="delta-j0-input", type="number", value=0.5, step=0.01, className="sidebar-input"),
                                    html.Label("%", className="inline-label")
                                ],
                            ),
                            html.Div(
                            ...
                            html.Div(
                                className="input-pair",
                                children=[
                                    html.Label("vmin =", className="inline-label"),
                                    dcc.Input(id="vmin-input", type="number", value=-1, step=0.1, className="sidebar-input"),
                                ],
                            ),
                            html.Div(
                                className="input-pair",
                                children=[
                                    html.Label("vmax =", className="inline-label"),
                                    dcc.Input(id="vmax-input", type="number", value=3.5, step=0.1, className="sidebar-input"),
                                ],
                            ),
...
def build_matrix_and_update(is_running, delta_j0, delta_s, vmin, vmax, trgr):
    if not is_running:
        raise dash.exceptions.PreventUpdate
    ...
    PAIR_WORKERS = int(globals().get("PAIR_WORKERS", 15))
    ...
    if isinstance(data_obj, list):
        ...
        im = ax.imshow(big_img, cmap="jet", origin="upper", vmin=vmin, vmax=vmax, interpolation="nearest")
        ...
        # Color scale log
        logs.append(f"🎨 Color scale: vmin={_fmt(vmin)}; vmax={_fmt(vmax)}")
